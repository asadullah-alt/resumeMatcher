...
@job_router.get(
    "/openjob",
    summary="Get job data without token verification",
)
async def open_job(
    request: Request,
    job_id: str = Query(..., description="Job ID to fetch data for"),
    ...
    """
    Retrieves job data by job_id without requiring a token.
    ...
    try:
        ...
        logger.error(f"Error fetching open job: {str(e)} - traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error fetching job data",
        )
